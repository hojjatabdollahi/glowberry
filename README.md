...
Enhanced background/wallpaper service with live shader support. Features include:
...
- **Live shader wallpapers** - GPU-rendered animated backgrounds using WGSL shaders
...
If packaging, use the `rootdir` variable to change the root path, in addition to the prefix: `just rootdir=debian/glowberry prefix=/usr install`.

To reduce compile times, either use `sccache`, or set `CARGO_TARGET_DIR` to a shared path and install with `sudo -E just install`.

## Debugging

To get debug logs from the service, first kill the `glowberry` process. Then launch it with `just run` to display backtraces and debug logs in the terminal.
